...
class Input extends FormComponent
{
    ...
    {
        return Str::of('text-wire_negative-900 dark:text-wire_negative-600 placeholder-wire_negative-300 dark:placeholder-wire_negative-500')
            ->unless($this->borderless, function (Stringable $stringable) {
                return $stringable
                    ->append(' border border-wire_negative-300 focus:ring-wire_negative-500 focus:border-wire_negative-500')
                    ->append(' dark:bg-wire_secondary-800 dark:border-wire_negative-600');
            });
    }

    protected function getDefaultColorClasses(): string
    {
        return Str::of('placeholder-wire_secondary-400 dark:bg-wire_secondary-800 dark:text-wire_secondary-400')
            ->append(' dark:placeholder-wire_secondary-500')
            ->unless($this->borderless, function (Stringable $stringable) {
                return $stringable
                    ->append(' border border-wire_secondary-300 focus:ring-wire_primary-500 focus:border-wire_primary-500')
                    ->append(' dark:border-wire_secondary-600');
            });
    }
