            ...
            <x-slot name="append">
                <div class="absolute inset-y-0 right-3 z-5 flex items-center justify-center">
                    <div @class([
                        ...
                        'text-wire_negative-400 dark:text-wire_negative-600' => $name && $errors->has($name),
                        'text-wire_secondary-400'                       => $name && $errors->has($name),
                    ])>
                        <x-dynamic-component
    ...
    <x-wireui::parts.popover :margin="(bool) $label" class="p-2.5">
        ...
        <ul class="mt-1 w-full h-64 sm:h-32 pb-1 pt-2 overflow-y-auto soft-scrollbar">
            <template x-for="time in filteredTimes">
                <li class="group rounded-md focus:outline-none focus:bg-wire_primary-100 hover:text-white
                            hover:bg-wire_primary-600 cursor-pointer select-none relative py-2 pl-2 pr-9
                            dark:hover:bg-wire_secondary-700"
                    :class="{
                        'text-wire_primary-600 dark:text-wire_secondary-400':   input === time.value,
                        'text-wire_secondary-700 dark:text-wire_secondary-400': input !== time.value,
                    }"
                    tabindex="0"
                    ...
                    <span
                        class="
                            absolute text-wire_primary-600 group-hover:text-white inset-y-0
                            right-0 flex items-center pr-4 dark:text-wire_secondary-400
                        "
