<div {{ $attributes->only(['class', 'wire:key'])->class('relative') }}
    ...
    <div class="relative">
                ...
                <div :class="{
                    ...
                    <template x-if="!config.multiselect">
                        <div @class([
                            ...
                            <span
                                class="truncate text-wire_secondary-700 dark:text-wire_secondary-400 text-sm"
                                x-show="!isEmpty()"
                        ...
                        <div class="absolute left-0 inset-y-0 pl-3 pr-14 w-full flex items-center overflow-hidden" x-on:click="toggle">
                            <div class="flex items-center gap-2 overflow-x-auto hide-scrollbar">
                                ...
                                @if (!$withoutItemsCount)
                                    <span
                                        class="inline-flex text-wire_secondary-700 dark:text-wire_secondary-400 text-sm"
                                ...
                                <div wire:ignore class="flex flex-nowrap items-center gap-1">
                                    <template x-for="(option, index) in selectedOptions" :key="`selected.${index}`">
                                        <span class="
                                                ...
                                                border border-wire_secondary-200 shadow-sm bg-wire_secondary-100 text-wire_secondary-700
                                                dark:bg-wire_secondary-700 dark:text-wire_secondary-400 dark:border-none
                                            ">
                                            <span style="max-width: 5rem" class="truncate" x-text="option.label"></span>

                                            <button
                                                class="shrink-0 h-4 w-4 flex items-center text-wire_secondary-400 justify-center hover:text-wire_secondary-500"
                ...
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 gap-x-2">
                    @if ($clearable && !$readonly && !$disabled)
                        <button
                            ...
                            <x-dynamic-component
                                :component="WireUi::component('icon')"
                                class="w-4 h-4 text-wire_secondary-400 hover:text-wire_negative-400"
                                name="x"
                            />
                        </button>
                    @endif
                    ...
                    <button tabindex="-1" x-on:click="toggle" type="button">
                        <x-dynamic-component
                            ...
                            {{ $name && $errors->has($name)
                                ? 'text-wire_negative-400 dark:text-wire_negative-600'
                                : 'text-wire_secondary-400'
                            }}"
                            :name="$rightIcon"
                        />
                    </button>
        ...
        @if ($hint)
            <label @if ($id) for="{{ $id }}" @endif class="mt-2 text-sm text-wire_secondary-500 dark:text-wire_secondary-400">
                {{ $hint }}
            </label>
        @endif
    </div>
    ...
    <x-wireui::parts.popover :margin="(bool) $label" root-class="sm:w-full">
        ...
        <div class="max-h-64 sm:max-h-60 overflow-y-auto overscroll-contain soft-scrollbar select-none"
            ...
            <div class="w-full h-0.5 rounded-full relative overflow-hidden"
                ...
                <div class="bg-wire_primary-500 h-0.5 rounded-full absolute animate-linear-progress"
                    style="width: 30%"
            ...
            @unless ($hideEmptyMessage)
                <div class="py-12 px-3 sm:py-2 sm:px-3 text-center sm:text-left text-wire_secondary-500 cursor-pointer"
