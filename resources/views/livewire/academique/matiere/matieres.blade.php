<section>
  ...
  <div class="tab-content mt-5 print:shadow-none print:m-0 print:p-5">
    @if($currentClasse)
      <div wire:loading.remove>
        <livewire:academique.matiere.matiere-par-classe :classe="$currentClasse" wire:key="liste-matiere-{{$currentClasse}}-classe" />
      </div>
      <div wire:loading class="flex justify-center w-full text-center">
        <x-loading />
      </div>
    @else
      <x-empty-state titre="Choisir une Classe" message="Pour en Afficher les matieres" />
    @endif
  </div>

  <div id="formulaire-creation-matiere" class="modal modal-slide-over print:hidden" data-tw-backdrop="static" tabindex="-1" aria-hidden="true">
  ...
  <div id="modal-show-matiere" class="modal print:hidden" data-tw-backdrop="static" tabindex="-1" aria-hidden="true">
