<section wire:init="loadData" class="col-span-12">
    ...
    function initListeEleveExamenBlanc(selector,info) {
      if ($(selector).length) {
              ...
              },
              {
                ...
                formatter(cell, formatterParams) {
                  let a =
                    ...
                    <a class="btn edit flex items-center mr-3 text-danger" 
                      onclick="
                        window.$wireui.confirmDialog({
                          title:'Confirmation',
                          description: \`Retirer l'eleve ${cell.getData().nom} ${cell.getData().prenoms} de l'examen ?\` ,
                          icon: 'question',
                          accept: {
                            label: 'Oui, Retirer',
                            color: 'warning',
                            execute: () => {Livewire.emit('deleteEleveFromRegistre',${cell.getData().id}) ;}
                          },
                          reject: {
                              label: 'Non',
                          }
                      ...
                      ">
                      <i data-feather="trash" class="w-4 h-4 mr-1"></i> Supprimer
                    </a>
                  </div>`);
                  return a[0];
                },
        ...
        $("#EleveTabulator-export-xlsx").on("click", function (event) {
          window.XLSX = xlsx;
          table.download("xlsx", "Liste-Eleve-Examen-Blanc.xlsx", {
            sheetName: "Registre Examen Blanc",
          });
        });
