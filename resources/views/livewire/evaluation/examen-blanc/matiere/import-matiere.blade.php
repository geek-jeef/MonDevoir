<section>
  <div class="mb-5">
    ...
    <form wire:submit.prevent="createFromExcel">
      <x-errors class="mt-3"/>
      <div class="mt-3">

  
      <form action="#" class="my-6" x-data="{ files: null }">
        <label class="border-2 border-gray-200 p-3 w-full block rounded cursor-pointer my-2" for="customFile2">
          <input type="file" class="sr-only" id="customFile2" x-on:change="files = Object.values($event.target.files)">
          <span x-text="files ? files.map(file => file.name).join(', ') : 'Choose file with reset...'"></span>
        </label>
        <button type="reset" @click="files = null" class="bg-gray-300 text-gray-800 px-3 py-1 rounded">Reset</button>
      </form>
      ...
      Cliquez ici pour choisir un fichier ou glissez-déposez un fichier | XLSX, CSV, TSV, ODS, XLS, SLK, XML, GNUMERIC, HTML | &lt; 4.00MB
      <div class="mt-3">
