  ...
  <div class="intro-y tab-content mt-5">
    <div class="grid grid-cols-12 gap-6 mt-5">
      <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <div class="hidden md:block mx-auto text-slate-500"> {{ $devoirs->links('components.pagination.custom-indicator') }} </div>
      ...
      <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">      
        ...
        <tbody>
          ...
          <tr class="intro-x">
            ...
            <td>
              <a href="javascript:;" class="font-medium whitespace-nowrap">{{ $devoir->nom_devoir}}</a>
            </td>
            <td class="text-center">{{$devoir->ponderation}}</td>
            <td class="text-center">{{$devoir->matiere->nom_matiere}}</td>
            @php
              $devoir_classe = $devoir->classe;
            @endphp
            <td class="text-center">{{$devoir_classe->cycle->nom_cycle}}</td>
            <td class="text-center whitespace-nowrap">{{$devoir_classe->nom_classe}}</td>
                ...
                <a class="flex items-center mr-3 text-primary whitespace-nowrap" href="javascript:;" onclick="
                  window.$wireui.confirmDialog({
                    ...
                    accept: {
                        ...
                        color: 'primary',
                        execute: () => { window.open(@js(route('devoir.show',$devoir->id)), '_blank').focus();},
                    },
                    reject: {
                        label: 'Non, Annuler',
                        method: 'cancel'
